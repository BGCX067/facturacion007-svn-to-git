...
where nombre_apellido like '%$nombre%'");
    ...
    public function listarProductosFactura($id_factura){
        $sql = 'SELECT pf.id, pf.cantidad_producto, pf.total_producto, pro.descripcion, pro.precio_venta
    ...
    public function buscar_producto($nombre) {
        ...
        $consulta = $this->db->ejecutarConsulta("SELECT * from productos where descripcion like '%$nombre%'");
        $consulta->setFetchMode(PDO::FETCH_ASSOC);
        $data = $consulta->fetchAll();
        echo json_encode($data);
    }
